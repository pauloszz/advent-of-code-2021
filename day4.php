...
for ($board = 0; $board < 100; $board++) {
	for ($boardLine = 0; $boardLine < 5; $boardLine++) {
		for ($i = 0; $i < 5; $i++) {
			array_push($boards["board" . $board]["row" . $boardLine], $boardNumbers[$i]);
		}
		array_splice($boardNumbers, 0, 5);
	}
}

foreach ($boards as $boardName => $board) {
	foreach ($board as $lineName => $boardLine) {
		if (strpos($lineName, "row") !== false) {
			for ($i = 0; $i < 5; $i++) {
				array_push($boards[$boardName]["col" . $i], $boardLine[$i]);
			}
		}
	}
}
...
foreach ($bingoNumbers as $calledNumber) {
	foreach ($boards as $boardName => &$board) {
		foreach ($board as $lineName => &$boardLine) {
			foreach ($boardLine as $key => &$boardNumber) {
				if ($calledNumber == $boardNumber) {
					$boardLine[$key] = 0;
					if (array_sum($boardLine) == 0) {
						$bingo = true;
						$bingoBoard = $boardName;
						$bingoNumber = $calledNumber;
						$bingoRowCol = $lineName;
						break;
					}
				}
			}
...
echo "Bingo with number " . $bingoNumber . " - " . $bingoBoard . " - " . $bingoRowCol . "<br>";
$boardSum = 0;

foreach ($boards as $boardName => &$board) {
	if ($boardName == $bingoBoard) {
		foreach ($board as $lineName => &$boardLine) {
			if (strpos($lineName, "row") !== false) {
				$boardSum += array_sum($boardLine);
			}
		}
	}
}

echo "Sum of uncalled numbers: " . $boardSum . "<br>";
echo "Score: " . ($boardSum * $bingoNumber);
...
echo "Last bingo with number " . $bingoNumber . " - " . $bingoBoard . " - " . $bingoRowCol . "<br>";
$boardSum = 0;
